---
title: "The Stourgate Bugle"
draft: false
---

**THE TRAGEDY OF STOURGATE**

What follows is an archive of thestourgatebugle.co.uk, taken offline following the horrific events that befell Stourgate. The captures below are what could be recovered.

Some material has been withheld out of respect for the dead and their families.

Captures are being released gradually as each one is checked and cleared.

<img src="/images/hero.jpg" alt="Stourgate seafront" class="front-hero">

Select a capture to view the site as it looked around that time. Dates are approximate. To best understand what happened in Stourgate we recommend you read each capture in full in chronological order.
